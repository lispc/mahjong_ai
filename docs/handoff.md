...
- 对手建模：阴性；
- 错题本蒸馏、自对弈赢家蒸馏：阴性；
- conv value net 精调：提升微弱（+10~20 Elo），不足以支撑 depth-2 搜索；
- **模型集成 / Model Soup：阳性**（当前最强候选）。

继续提升的候选方向（按推荐顺序）：

1. **扩大模型集成规模**：把 128k 多个 epoch（1–30）与 best、error-focused、winner-only 等做 weighted soup / greedy soup，自动搜索最优权重组合。
2. **用 soup 当教师做下一轮蒸馏**：生成 soup 自对弈数据或搜索轨迹，训练单一模型，看能否把集成收益蒸馏回一个网络。
3. **系统性超参/训练协议搜索**：在 32k/128k 数据上 grid search optimizer、lr schedule、batch size、weight decay、dropout、label smoothing。
...
**当前状态**：
- 最强候选：`Hybrid-Soup2`（`output/nn_full_action_soup_best_epoch7.pt`）。
- 待确认：是否将其提升为新的 `nn_full_action_best.pt`（需用户授权覆盖旧 best）。
- 下一步建议：先做 **更大规模的 greedy soup / multi-epoch soup**，若还能提升再固定为新 best。
