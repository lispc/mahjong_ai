...
- 当前最强候选从 Soup2 升级到 `Hybrid-SoupDistilled`。
- 待用户确认是否把 `output/nn_full_action_soup_distilled.pt` 提升为新的 `output/nn_full_action_best.pt`。
...
- 模型集成 / Model Soup：**阳性**；
- Soup 蒸馏回单一模型：**阳性**（当前最强候选）。

继续提升的候选方向（按推荐顺序）：

1. **把 SoupDistilled 提升为新 best 并迭代**：固定新 best 后，用它再生成教师数据、再 soup、再蒸馏，看能否继续 bootstrap。
2. **更大规模的 greedy soup**：把 128k 多个 epoch（1–30）与 best、error-focused、winner-only 等做 weighted soup / greedy soup，自动搜索最优权重组合，再蒸馏。
3. **系统性超参/训练协议搜索**：在 32k/128k/soup 数据上 grid search optimizer、lr schedule、batch size、weight decay、dropout、label smoothing。
4. **更大网络 + 更强正则化**：把 `TileConvNet` 从 128/6/512 放大到 256/8/1024 或加 SE/attention。
5. **人类/高水平对局数据**：引入外部数据打破当前自我蒸馏循环。
6. **动作空间扩展 / 搜索改造**：完整动作空间 response head 已有，但“吃/碰/杠/暗杠/报听”的内部决策仍是简化版；或把 search 从 depth-1 expectimax 改成 depth-2 + 更强 value net（前提是 value net 先取得突破）。

**当前状态**：
- 最强候选：`Hybrid-SoupDistilled`（`output/nn_full_action_soup_distilled.pt`）。
- 待确认：是否将其提升为新的 `nn_full_action_best.pt`（需用户授权覆盖旧 best）。
- 下一步建议：用户确认后，用新 best 再做一轮「生成数据 → soup → 蒸馏」迭代。
