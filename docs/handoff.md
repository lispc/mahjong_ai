...
1. **模型集成（Model Ensemble / Soup）**：把 `nn_full_action_best.pt`、128k epoch 7、error-focused、selfplay-winner 等 checkpoint 做权重平均或投票，零训练成本验证；若有效再固定为新的 best。
2. **系统性超参/训练协议搜索**：在 32k 数据上 grid search optimizer（AdamW / Lion）、learning rate schedule（cosine / plateau / warm restart）、batch size、weight decay、dropout、label smoothing，看是否只是训练欠调优。
3. **更大网络 + 更强正则化**：把 `TileConvNet` 从 128/6/512 放大到 256/8/1024 或加 SE/attention，配合更多数据/更强制约再训。
4. **人类/高水平对局数据**：当前所有数据都来自 eval2/BeliefExp/NN 自身，分布互相蒸馏已收敛；引入外部强对局数据可能打破循环。
5. **动作空间扩展 / 搜索改造**：完整动作空间 response head 已有，但“吃/碰/杠/暗杠/报听”的内部决策仍是简化版；或把 search 从 depth-1 expectimax 改成 depth-2 + 更强 value net。

**当前状态**：best 仍为 `Hybrid-FullAction-32k`（`output/nn_full_action_best.pt`）。下一条具体实验建议先做 **模型集成**，若无效再考虑超参搜索或更大网络。
