...
- 16000 局数据 + T=8 是当前最佳组合，继续放大到 32000 局或继续调 T 的收益预计边际递减。
...
当前决定：**纯 BeliefExp 教师 + big-size 网络 + 16000 局数据 + T=8 是当前最强组合**。`Hybrid-BE16k_t8` 在 2000 局确认 benchmark 中取得 **胜率 25.8%、点炮 16.3%、Elo 1581**。后续可选：
1. **继续放大到 32000 局数据 + T=8 训练**（预计收益边际递减，需评估成本）；
2. 若不再继续投入，当前最佳配置锁定为 `hybrid:BE16k_t8:output/nn_conv_bc_beliefexp_trace_16000_big_t8.pt:beliefexp`。
