...
- **当前最佳候选**：`hybrid:BE8k_t8:output/nn_conv_bc_beliefexp_trace_8000_big_t8.pt:beliefexp`
- **性能**：2000 局确认胜率 **25.6%**，点炮 **16.0%**，Elo **1618**
- **上一版本候选**：`hybrid:BE8k_t4:output/nn_conv_bc_beliefexp_trace_8000_big_t4.pt:beliefexp`（2000 局确认 Elo 1520）
...
| **Search trace distillation** | **Hybrid-BE8k_t8（8000局 BeliefExp教师，128/6/512，T=8）：2000局确认胜率 25.6%、点炮 16.0%、Elo 1618** | **是（当前最佳）** |
| **Safety-aware 报听** | 多阈值均降低胜率、升高点炮 | **否** |

**当前最佳候选**：
- **当前最佳**：`hybrid:BE8k_t8:output/nn_conv_bc_beliefexp_trace_8000_big_t8.pt:beliefexp`
...
- **Search trace distillation 是有效提升路线，纯 BeliefExp 教师 + 大网络 + 更 soft target（最终 T=8）+ 充足数据是当前最强组合**：`Hybrid-BE8k_t8` 在 2000 局确认中胜率 25.6%、点炮 16.0%、Elo 1618；
- **Safety-aware 报听（当前实现）无效**：限制报听反而降低胜率、升高点炮；
- 8000 局数据 + T=8 已接近饱和，16000 局数据生成中，若继续提升则最终配置可能再更新。
...
- 关键成功因素：**教师必须本身就在目标分布上强**；纯 BeliefExp 教师每步都搜索，提供了最一致、最强的 soft target；数据量充足后，**更大网络 + 更 soft target（最终 T=8）能更好地消化搜索信息**。

### 结论

- **搜索轨迹蒸馏成功，且纯 BeliefExp 教师是当前最强教师**；
- **`Hybrid-BE8k_t8` 在 2000 局确认中胜率 25.6%、点炮 16.0%、Elo 1618，是当前最佳候选**；
- 8000 局数据 + big-size 网络 + T=8 是当前最佳组合，16000 局数据生成中，若继续提升则最终配置可能再更新。
...
当前决定：**纯 BeliefExp 教师 + big-size 网络 + 8000 局数据 + T=8 是当前最强组合**。`Hybrid-BE8k_t8` 在 2000 局确认 benchmark 中取得 **胜率 25.6%、点炮 16.0%、Elo 1618**，显著优于 T=4（Elo 1520）。后续可选：
1. **继续用 16000 局数据 + T=8 训练最终模型**（已在生成中，预计收益边际递减）；
2. 若 16000 局无提升，当前最佳配置锁定为 `hybrid:BE8k_t8:output/nn_conv_bc_beliefexp_trace_8000_big_t8.pt:beliefexp`。

Safety-aware 报听（当前 dealin-head 风险估计实现）已证伪，不再继续。

如果用户不继续投入，当前保留 `Hybrid-BE8k_t8` 为最稳健配置。
