...
- `beliefexpused` = BeliefExpectimaxAgent(used_aware_eval2=True)（eval2 真正使用已见牌；2026-07-20 A/B 阴性，勿作强度方向）
- `baselinepc06` = BaselinePairCoefAgent（pair_coef=0.6；2026-07-20 A/B 与 1.0 无差异）
...
10. ~~**`tests/legacy_test.py` 已知失败~~（2026-07-17 已修）：`test_select` 断言已改为集合比较（tie-break 顺序不稳定是 Cython eval2 集成所致，同分值无强度差异），当前全绿。勿改动 legacy eval 语义（Baseline 依赖 `algo.select`）。
...
13. ~~**Hybrid 的 melds 列表 quirk**~~（**2026-07-19 已修**）：历史上 Hybrid 把同一 melds 列表共享给 nn_agent/belief_agent 且 `add_meld` 被三个组件各调一次 → 每个副露出现 3 次。现已改为只追加一次（列表本就共享）。`scripts/rl/peng_paired_eval.py::_inject` 仍按旧的三重表示复刻历史状态（该管线已判死，无碍）。
...
18. ~~**BeliefExp 的 eval2 实为「空 Context」（2026-07-20 发现）**~~（**同日已测并关闭**）：`algo.eval2` Cython 快路径要求 context 有 `all_tiles_as_dict` 方法才使用 `used`，legacy `context.Context` 没有 → BeliefExp 精心维护的 `used` 被**静默忽略**。修复变体 `beliefexpused`（`UsedAwareContext`）1000p duplicate vs Baseline **−2.6% [−5.0,−0.2]**（点炮 −2.6pp 但胜率显著负，「更守但不更强」第四次同构阴性）→ **不修复即现状语义，jax 侧无需同步**。同日 pair_coef A/B：Cython 1.0 vs config 0.6 = −1.0% [−2.5,+0.5] 噪声内，**维持 de-facto 1.0 不修**。详见 `docs/reports/used-paircoef-ab-0720.md`。
