...
用法：
    PYTHONPATH=. python3 scripts/rl/gen_beliefexp_trace_data.py \
        output/nn_teacher_beliefexp_trace_500.npz 500 16 15000000 --save-every 250

参数：out_path n_games n_workers seed_base [--save-every N] [--resume]
"""
...
def _empty_arrays():
    return {'X': None, 'y': None, 'scores': None, 'dangers': None,
            'selected_value': None, 'has_trace': None, 'dealin': None, 'v': None}
...
def _stack_chunk(chunk):
    ...
    DANG = np.stack([s[3] for s in chunk])
    SV = np.array([s[4] for s in chunk], dtype=np.float32)
    HT = np.array([s[5] for s in chunk], dtype=np.float32)
    D = np.stack([s[6] for s in chunk])
    v = np.array([s[7] for s in chunk], dtype=np.float32)
    return {'X': X, 'y': y, 'scores': S, 'dangers': DANG, 'selected_value': SV,
...
def main():
    ...
    parser.add_argument('out_path', default='output/nn_teacher_beliefexp_trace.npz')
    ...
    args_list = [(i, seed_base + i, completed) for i in range(start_i, total_games)]
    ...
    print(f'Total {acc["X"].shape[0] if acc["X"] is not None else 0} samples '
    ...
    print(f'Saved {out_path}: X={acc["X"].shape}, y={acc["y"].shape}, v={acc["v"].shape}')
