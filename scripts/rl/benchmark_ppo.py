...
BEST_PATH = 'output/nn_full_action_best.pt'
NAMES = ['PPO', 'BC32k', 'V3']
...
def make_best():
    _set_env()
    return HybridNNBeliefAgent('BC32k', nn_model_path=BEST_PATH,
                               belief_kind='beliefexp', device='cpu',
                               temperature=None, verbose=False)
...
def main():
    ...
    ap.add_argument('--seed', type=int, default=3900000)
    args = ap.parse_args()
    import multiprocessing as mp
    mp.set_start_method('spawn', force=True)
    _set_env()

    results = run_tournament([make_ppo, make_best, make_v3, make_best],
