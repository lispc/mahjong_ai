# -*- coding: utf-8 -*-
"""Benchmark 128k BC Epoch1 vs PPO vs BC32k vs V3."""
import os
import argparse
...
EPOCH1_PATH = 'output/nn_full_action_128000_epoch_01.pt'
PPO_PATH = 'output/nn_full_action_ppo.pt'
BC32K_PATH = 'output/nn_full_action_best.pt'
NAMES = ['BC128k_E1', 'PPO', 'BC32k', 'V3']
...
def make_epoch1():
    _set_env()
    return HybridNNBeliefAgent('BC128k_E1', nn_model_path=EPOCH1_PATH,
...
def main():
    ...
    ap.add_argument('--seed', type=int, default=4000000)
    args = ap.parse_args()
    import multiprocessing as mp
    mp.set_start_method('spawn', force=True)
    _set_env()

    factories = [make_epoch1, make_ppo, make_best, make_v3]
    ...
    print(f"{'Agent':<12} {'games':<8} {'win':<8} {'self':<8} {'ron':<8} {'deal-in':<10} {'draw':<8} {'Elo':<8} {'avg_ms':<10}")
    for n in NAMES:
        print(f"{n:<12} {m[n]['games']:<8} {m[n]['win_rate']:<8.3f} {m[n]['self_rate']:<8.3f} "
